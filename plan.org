...
* Things I need to know ?
 ...
 - livegrep: can it regex across lines
   + no
 - How much slower is git cloning --mirror than normal git clone (both set to depth 1)
...
* TOD0
  ...
  3) build script for pulling repos limited on the livegrep tool to 800~900 (i think) pull repos with depth 1 then have to build json file for codesearch (2)
  ...
  4) Limit problem on results 8000 - 11600 seems to vary a bit something to do with message size being larger than max (3)
  5) Generic makefile to build projects that are return by regex search (4)
