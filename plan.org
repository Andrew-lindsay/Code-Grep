...
* Tasks
 ...
 4) Confirm Number of repositories GitHub Api can return (1000 get more by using star count diff search) (done scripted)  
 5) install livegrep on own machine using docker (stress test) (problematic)
 ...
 6) install on server using docker or install natively on server 
